...
def init_db_seed() -> None:
    """Создаёт таблицы и минимальные данные (роль и пользователя user/qwerty), если их нет."""
    db.create_all()
    # Роль по умолчанию
    role = Role.query.filter_by(name='Пользователь').first()
    if role is None:
        role = Role(name='Пользователь', description='Базовая роль пользователя')
        db.session.add(role)
        db.session.commit()
    # Пользователь user/qwerty
    user = User.query.filter_by(username='user').first()
    if user is None:
        user = User(username='user', last_name=None, first_name='User', middle_name=None, role=role)
...
@login_required
def visits():
    """Показывает количество посещений текущим пользователем страницы /visits."""
    count = session.get('visits_count', 0) + 1
    session['visits_count'] = count
    return render_template('visits.html', title='Счётчик посещений', count=count)
...
def user_view(user_id: int):
    """Просмотр данных пользователя (доступен всем)."""
    u = (
        db.session.query(
            User.id,
            User.username,
