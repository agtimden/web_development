...
class SimpleUser(UserMixin):
    """Простая in-memory модель пользователя для демонстрации."""

    def __init__(self, user_id: str):
        self.id = user_id


# Единственный пользователь из ТЗ
...
def auth():
    ...
    if request.method == 'POST':
        ...
        if user_record and user_record['password'] == password:
            print("12222222222")
            login_user(SimpleUser(username), remember=remember)
            flash('Вы успешно вошли в систему.', 'success')
            next_url = request.args.get('next')
            print(next_url)
            return redirect(next_url or url_for('visits'))
        else:
            error = '1111111111Неверный логин или пароль.'
